...
# W3-Ex2 (Josep Coll): which server script to mount, so the same agent can be
# pointed at the 1-tool server or at the 6-tool one without editing code.
SERVER_SCRIPT = os.environ.get("SERVER_SCRIPT", "server.py")
SERVER = StdioServerParameters(command=sys.executable, args=[SERVER_SCRIPT])
